...
<!-- stores --> 
	...
	<xsl:template match="obj" mode="model"><!--{{{-->
		...
		Ext.define( '<xsl:value-of select="concat($application_name,'.',$module_name,'.',$obj_name)"/>', {
			extend: 'Ext.data.Model'
			,class_name: '<xsl:value-of select="@name"/>'
			,module: '<xsl:value-of select="$module_name"/>'
			,proxy: { type: 'xpproxy', class_name: '<xsl:value-of select="@name"/>' }
	...
	<xsl:template match="obj" mode="store"><!--{{{-->
		...
		Ext.define('<xsl:value-of select="concat($application_name,'.store.',$obj_name)"/>', Ext.apply({ 
			...
			,model: '<xsl:value-of select="concat($application_name,'.',$module_name,'.',$obj_name)"/>'
	...
	Ext.define('<xsl:value-of select="concat($application_name,'.store.',../from,'_',@name)"/>', {
		...
		,model:'<xsl:value-of select="concat($application_name,'.',$module_name,'.',../from,'_',@name)"/>'
	...
	<xsl:template match="query" mode="model_eh"><!--{{{-->
		...
		<xsl:variable name="parent_obj_name" select="../../../@name"/>
			...
			Ext.define( '<xsl:value-of select="concat($application_name,'.',$module_name,'.',../from,'_',@name)"/>', {
extend: 'Ext.data.Model'
,class_name: '<xsl:value-of select="@name"/>'
,module: '<xsl:value-of select="$module_name"/>'
,proxy: { type: 'xpproxy', class_name: '<xsl:value-of select="$obj_name"/>', 
	...
	<xsl:template match="obj" mode="associations"><!--{{{-->
		...
		<xsl:if test="foreign_key">
			...
			{	name:'<xsl:value-of select="$parent"/>'
				,model:'<xsl:value-of select="concat($application_name,'.',$module_name,'.',$parent)"/>'
		...
		<xsl:if test="count(obj/foreign_key)">
			...
			{	name:'<xsl:value-of select="$child"/>'
				,model:'<xsl:value-of select="concat($application_name,'.',$module_name,'.',$child)"/>'
				,type:'<xsl:value-of select="$assoc_type"/>'
				/* ,autoload: true */
