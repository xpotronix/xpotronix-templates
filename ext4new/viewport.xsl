...
<html>
	...
	<xsl:call-template name="app-config-root"/>
	...
	<xsl:if test="$menu_bar='true'">
		<xsl:apply-templates select="//xpotronix:session/menu" mode="config"/>
	</xsl:if>
	...
	<xsl:template name="app-config-root"><!--{{{-->
	...
	<xsl:template match="menu" mode="config"><!--{{{-->
	App.menu = new Ext.Toolbar( <xsl:apply-templates select="."/> );
	/*
	Ext.Ajax.request({
