...
<f:model>
	...
	<xsl:template match="class" mode="grid"><!--{{{-->
			...
			<table class="tbl" border="1" cellpadding="2" cellspacing="1" width="100%">
	...
	<xsl:template match="attr" mode="table_body"><!--{{{-->
		...
		<xsl:if test="@display!='hide' or not(@display)">
		<xsl:element name="td">
			<xsl:if test="@type='int' or @type='double' or @type='decimal' or @type='float'">
	...
	<xsl:template name="wellcome"><!--{{{-->
         ...
         <tr>
         <td width="100%"><h1>Fotoshow Temporada 2008</h1>
	 </td>
         </tr>
         </table>
