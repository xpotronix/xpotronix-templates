...
<xsl:template match="obj" mode="get_feat"><!--{{{-->
		...
		<xsl:variable name="feats" select="//xpotronix:session/feat"/>
	...
	<xsl:template name="user-session">{<xsl:for-each select="//xpotronix:session/users/*"><xsl:value-of select="name()"/>:<xsl:apply-templates select="." mode="json-value"/><xsl:if test="position()!=last()">,</xsl:if></xsl:for-each>}
	</xsl:template>
